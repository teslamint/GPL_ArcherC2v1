             ...
             av_codecs_t *codecs)
{
  ...
  if (st == CT_AAC)
  {
    ...
    /*ffmepg didnot get enough info? Added by LI CHENGLONG , 2011-Dec-07.*/
...
probe2_mpeg4()
{
	return aac_profiles_mapping[0].profile;
}

dlna_registered_profile_t dlna_profile_audio_mpeg4 = {
  .id = DLNA_PROFILE_AUDIO_MPEG4,
  .class = DLNA_CLASS_AUDIO,
  .extensions = "aac,mp4,m4a",
   /*识别文件中的信息是在ffmegp中实现的,对于某些媒体格式需要特殊的库支持,
   在ffmpeg编译时可以指定,Added by LI CHENGLONG , 2011-Dec-07.*/
  .probe = probe_mpeg4,
  .probe2 = probe2_mpeg4,
  .next = NULL
};
