...
/* Remove to sdp_neg.h, by sirrain zhang */
#if 0
/**
				    ...
				    const pjmedia_sdp_session *local)
{
     ...
     */
     #if 0 /*ycw-pjsip-t38. I don't known why to copy the old media.*/
					  ...
					  unsigned fmt_idx)
{
	...
	if (p) {
	    ...
	    *(p + STR_OCTET_ALIGN.slen) = (char)(octet_align? '0' : '1');
	    ...
	    pj_ansi_snprintf(buf, MAX_FMTP_STR_LEN, "%.*s;octet-align=1",
			     (int)fmtp.fmt_param.slen, pj_strnull(fmtp.fmt_param.ptr));
	...
	pj_ansi_snprintf(buf, MAX_FMTP_STR_LEN, "%.*s octet-align=1",
			 (int)media->desc.fmt[fmt_idx].slen,
		         pj_strnull(media->desc.fmt[fmt_idx].ptr));
	attr->value = pj_strdup3(pool, buf);
				     ...
				     pj_bool_t allow_asym)
{
     ...
     */
    if (pjmedia_sdp_transport_cmp(&answer->desc.transport, &offer->desc.transport) 
				!= PJ_SUCCESS)
    {
		...
		for (i=0; i<offer->desc.fmt_count;)
		{
		   ...
		   pt = pj_strtoul(fmt);
			     ...
			     (ar.param.slen==1 && *ar.param.ptr=='1')))
			{
			    ...
			    {
								if (match_amr(offer, i, answer, j, PJ_FALSE, NULL))
				    break;
						    }
							 else
							 {
			       ...
			       pjmedia_sdp_media **p_answer)
{
    ...
    pj_str_t *vbd_pt = NULL; /* ycw-pjsip: vbd support*/
    pjmedia_sdp_attr *vbd_attr_rtpmap = NULL;
    pjmedia_sdp_attr *vbd_attr_gpmd = NULL;
    unsigned found_vbd = 0;

 	 /*ycw-pjsip.*/
 	 pjmedia_sdp_attr* oa_televt = NULL;
	 pjmedia_sdp_rtpmap or_televt = {{0,0}, {0,0}, 0, {0,0}};
	 pjmedia_sdp_attr* aa_televt = NULL;
	 pjmedia_sdp_rtpmap ar_televt = {{0,0}, {0,0}, 0, {0,0}};
	 pjmedia_sdp_attr* attr_afmtp = NULL;
	 pjmedia_sdp_fmtp afmtp = {{0,0},{0,0}};
	 enum {ATTR_TEL_EVENT_VALUE_LEN = 160};
	 char buf[ATTR_TEL_EVENT_VALUE_LEN] = {0};
	 pj_bool_t offer_has_telephone_event = PJ_FALSE;
	 pj_bool_t answer_has_telephone_event = PJ_FALSE;
    ...
    for (i=0; i<master->desc.fmt_count; ++i)
	 {
        ...
        if (!found_vbd) { /* ycw-pjsip: vbd support*/
            vbd_attr_gpmd = pjmedia_sdp_media_find_attr2(master, "gpmd",
                            &master->desc.fmt[i]);
            if (vbd_attr_gpmd) {
                vbd_attr_rtpmap = pjmedia_sdp_media_find_attr2(master, "rtpmap", 
                                    &master->desc.fmt[i]);
                vbd_pt = (pj_str_t *)&master->desc.fmt[i];
                found_vbd = 1;
            }
        }
		...
		for (j=0; j<slave->desc.fmt_count; ++j) {
			...
			if (!pj_stricmp(&or_.enc_name, &lr.enc_name) &&
			    ...
			    pj_strtoul(&lr.pt) >= 96 && /*ycw-pjsip-g726*/
    ...
    */
    ...
    for (i=0; i<pt_answer_count; ++i)
	 {
		...
		for (j=i; j<answer->desc.fmt_count; ++j)
		{
	   	if (!pj_strcmp(&answer->desc.fmt[j], &pt_answer[i]))
			{
            pjmedia_sdp_attr *a;
            /* ycw-pjsip vbd support: remove gpmd for this format, add match vbd below */
            a = pjmedia_sdp_media_find_attr2(answer, "gpmd", 
                &answer->desc.fmt[j]);
            if (a)
				{
                pjmedia_sdp_media_remove_attr(answer, a);
    ...
    if (found_vbd && pj_strtoul(vbd_pt) < 96/*not dynamic*/)
	 { /*ycw-pjsip. vbd support*/
        for (i=0; i<slave->desc.fmt_count; ++i)
		  {
            if (pjmedia_sdp_media_find_attr2(slave, "gpmd", &slave->desc.fmt[i]))
                break;
        }
    	  if (i < slave->desc.fmt_count)
		  {
        		for (i = 0; i < answer->desc.fmt_count; i++)
				{
            	if (!pj_strcmp(&answer->desc.fmt[i], vbd_pt)) break;
        		}
        		if (i == answer->desc.fmt_count)
				{ 
					// not exist
            	pj_strdup(pool, &answer->desc.fmt[answer->desc.fmt_count++], vbd_pt);
            	if (vbd_attr_rtpmap) answer->attr[answer->attr_count++] = pjmedia_sdp_attr_clone(pool, vbd_attr_rtpmap);
            	answer->attr[answer->attr_count++] = pjmedia_sdp_attr_clone(pool, vbd_attr_gpmd);
        		}
        		else
				{ 
					// fmt contains vbd pt
            	pjmedia_sdp_attr *a;
            	a = pjmedia_sdp_media_find_attr2(answer, "gpmd", &answer->desc.fmt[i]);
            	if (!a)
               	answer->attr[answer->attr_count++] = pjmedia_sdp_attr_clone(pool, vbd_attr_gpmd);
        		}
    		}
	...
	/*ycw-pjsip*/
	 ...
	 for (i = 0; i < offer->desc.fmt_count; ++i)
	 {
		...
		oa_televt = pjmedia_sdp_media_find_attr2(offer, "rtpmap", &offer->desc.fmt[i]);
		if (!oa_televt)
		{
		    pj_assert(!"Bug! Offer should have been validated");
		    return PJMEDIA_SDP_EMISSINGRTPMAP;
		}
		pjmedia_sdp_attr_get_rtpmap(oa_televt, &or_televt);

		if (!pj_stricmp2(&or_televt.enc_name, "telephone-event"))
		{
			offer_has_telephone_event = PJ_TRUE;
			break;			
		}		
	 }
	 ...
	 for (i = 0; i < answer->desc.fmt_count; ++i)
	 {
	 	if (pj_isdigit(*answer->desc.fmt[i].ptr))
	 	{
	 		unsigned pt;
			pt = pj_strtoul(&answer->desc.fmt[i]);
			if (pt < 96) continue;
	 	}
		
	 	aa_televt = pjmedia_sdp_media_find_attr2(answer, "rtpmap", &answer->desc.fmt[i]);
		if (!aa_televt)
		{
		    pj_assert(!"Bug! Answer should have been validated");
			 return PJMEDIA_SDP_EMISSINGRTPMAP;
		}
		pjmedia_sdp_attr_get_rtpmap(aa_televt, &ar_televt);

		if (offer_has_telephone_event && !pj_stricmp2(&ar_televt.enc_name, "telephone-event"))
		{
			answer_has_telephone_event = PJ_TRUE;
			if (pj_stricmp(&ar_televt.pt, &or_televt.pt))
			{
				memset(buf, 0, sizeof(buf));
				pj_ansi_snprintf(buf, ATTR_TEL_EVENT_VALUE_LEN,
											"%.*s %s", (int)or_televt.pt.slen, pj_strnull(or_televt.pt.ptr),
											"telephone-event/8000");
				pj_strdup2_with_null(pool, &aa_televt->value, buf);
			}
	...
	for (i = 0; i < answer->desc.fmt_count; ++i)
	{
		...
		attr_afmtp = pjmedia_sdp_media_find_attr2(answer, "fmtp", &answer->desc.fmt[i]);
		if (!attr_afmtp)
		{
		    continue;
		}

		if (pjmedia_sdp_attr_get_fmtp(attr_afmtp, &afmtp) != PJ_SUCCESS) 
		{
			continue;
		}

		if (answer_has_telephone_event && !pj_stricmp(&afmtp.fmt, &ar_televt.pt))
		{
			memset(buf, 0, sizeof(buf));
			pj_ansi_snprintf(buf, ATTR_TEL_EVENT_VALUE_LEN, "%.*s %s", 
					(int)or_televt.pt.slen, pj_strnull(or_televt.pt.ptr), "0-15");
			pj_strdup2_with_null(pool, &attr_afmtp->value, buf);
		}
	...
	for (i = 0; i < answer->desc.fmt_count; ++i)
	{
		if (answer_has_telephone_event && !pj_stricmp(&answer->desc.fmt[i], &ar_televt.pt))
		{
			memset(buf, 0, sizeof(buf));
			pj_ansi_snprintf(buf, ATTR_TEL_EVENT_VALUE_LEN, "%.*s", (int)or_televt.pt.slen, 
									pj_strnull(or_televt.pt.ptr));
			pj_strdup2_with_null(pool, &answer->desc.fmt[i], buf);
		}
	}
    ...
    for (i=0; i<offer->media_count; ++i)
	 {
		...
		if (PJ_FALSE == enableT38 && (pj_strcmp2(&om->desc.media, "image") == 0))
		{
			continue;
		}
		...
		for (im=NULL, j=0; j<initial->media_count; ++j)
		{
	   	im = initial->media[j];
					...
					media_used[j] == 0)
	    	{
				...
				status = match_offer(pool, prefer_remote_codec_order, om, im, &am);
				if (status == PJ_SUCCESS)
				{
		    		/* Mark media as used. */
		    		media_used[j] = 1;
			 ...
			 /*ycw-pjsip. t38*/
			 else if (pj_strcmp2(&om->desc.media, "image") == 0)
			 { // t38
	            am = pjmedia_sdp_media_clone(pool, om);
	            am->conn = NULL; /* session-level */
	            am->desc.port = im->desc.port;
	            break;
	        }
