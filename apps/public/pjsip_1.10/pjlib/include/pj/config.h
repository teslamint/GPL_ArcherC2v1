...
/**
*	If this macro's value is 0, DSP will sends and receives RTP/RTCP data from network 
*	directly, not transits by pjSIP. Or, RTP/RTCP data will transit by pjSIP.
*/
#define PJ_MEDIA_TRANSIT_BY_PJSIP 0
...
/**yuchuwei
* brief support metabolic RTP/RTCP port
*/
#	ifndef SUPPORT_METABOLIC_MEDIA_PORT
#define	SUPPORT_METABOLIC_MEDIA_PORT	1
#	endif


/* 
 * brief	Support that user can make ip call even though there is no account.
 */
#define SUPPORT_IPCALL_NO_ACCOUNT 1

/* 
 * brief	Support PRESENCE
 *			Default:0(No)
 *	Author: yuchuwei@20120413
 */
#	ifndef SUPPORT_PRESENCE
#	define SUPPORT_PRESENCE			0
#	endif

/* 
 * brief	Support Instant Message(IM)
 *			Default:0(No)
 *	Author: yuchuwei@20120412
 */
#	ifndef SUPPORT_IM
#	define SUPPORT_IM					0
#	endif
...
/* 
 * brief	RTT(Round-Trip time)，往返时延，自发出Register请求
 *			至接收到服务器的第一个响应(不管什么响应)的时间。
 *			以此区分账号注册不同的SIP Server的速率，作为呼出时
 *			选择账号的一个参考因素。
 *			由于目前注册账号使用的是单线程，无法做到并发，导致计算
 *			的RTT无法如实反映账号注册的时间，所以这里不启用。
 */
#	ifndef SUPPORT_ACCOUNT_RTT
#	define SUPPORT_ACCOUNT_RTT		0
#	endif


/* 
 * brief	support RFC3960, which the Spanish Telefonica require.
 */
#ifdef INCLUDE_TFC_ES
#	ifndef PJ_RFC3960_SUPPORT
#	define PJ_RFC3960_SUPPORT	0
#	endif
#else
#	ifndef PJ_RFC3960_SUPPORT
#	define PJ_RFC3960_SUPPORT	0
...
/* 
 * brief	CM sends a fake call request to pjSIP every once in
 *			a while, and then pjSIP will try to simulate an out 
 *			call or an incoming call generation(only generate 
 *			some objects, not send anything).The purpose of this
 *			mechanism is to judge the memory is enough or not.
 *			But, the hazard is that when this mechanism is in 
 *			progress(it takes some memory), a true out call or 
 *			incoming call happens and the remining memory is not
 *			enough, the true call will faild.
 *			
 */
#define PJ_FAKE_CALL_SUPPORT 1
