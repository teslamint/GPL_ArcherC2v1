	...
	case OPT_NEXT_ENDPT:
		{
			...
			{
				PJ_LOG(1, (THIS_FILE, "CM only has %d endpoint, we can not add three or more endpoint!"));
				return -1;
			}
			cfg->usbvmEndpt_cnt++;
		}
...
#	if 0
	...
	if (uri_to_call->slen)
	{
		/*current_acc还要吗?考虑去掉这一个，要包括前面命令行选项的分析中设置uri_to_call
		部分*/
		pjsua_call_make_call( current_acc, uri_to_call, -1, 0, NULL, NULL, NULL);
	}
#	endif
...
#	endif
				...
				break;
			/* 这里不加宏，因为摘机必须执行下面的代码，且摘机不可能再发CMSIP_REQUEST_USBVM_EXIT*/
			/* 相当于对摘机事件单独处理 */
...
#	endif
#	if defined(SUPPORT_PRESENCE) && SUPPORT_PRESENCE!=0
		/**需要注意:如果启用了PRESENCE,pjsua_acc_add函数中
		*要求发出了REGISTER请求，那么很可能立即切换
		*线程到响应的处理线程，由于注册不成功将
		*default_acc赋值-1，然后再切换到本线程执行，这
		*时current_acc得到的值是-1，从而导致ASSERT的产生。
		*如果以后要启用PRESENCE,建议将pjsua_acc_set_onlinestatus
		*函数与pjsua_acc_add函数合并，从而避免不同步的问
		*题产生。
		*/
