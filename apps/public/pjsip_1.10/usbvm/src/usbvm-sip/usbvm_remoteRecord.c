...
static void remoteRecThread( void *context )
{
	...
	sem_wait((sem_t *)&g_usbvm_recordHandle[recordId].semRecord);
	while (1)
	{
...
{	
	...
	sprintf(l_recFilePathName[g_usbvm_recordHandle[recordId].endpt],
		"%s%d/", USB_REC_FILE_DIR, g_usbvm_recordHandle[recordId].endpt);
	...
	sprintf(strchr(l_recFilePathName[g_usbvm_recordHandle[recordId].endpt], '\0'), "%08x", g_usbvm_recordHandle[recordId].fileTimName);
	MARK_PRINT("start record fileName:%s\n",l_recFilePathName[g_usbvm_recordHandle[recordId].endpt]);
...
#endif
		memset(cmd, 0, sizeof(cmd));
		sprintf(cmd,"rm -f %s", l_recFilePathName[g_usbvm_recordHandle[recordId].endpt]);
		system(cmd);
	...
	/* close old fp in 'g_usbvm_recordHandle' if it is used,before open new fp */
		...
		fseek(g_usbvm_recordHandle[recordId].outputFp, sizeof(USBVM_REC_FILE_NODE), SEEK_SET);
	}
	/* add the file header's length */
	g_usbvm_recordHandle[recordId].fileSize += sizeof(USBVM_REC_FILE_NODE);
...
/*20130618_update*/
				if (fwrite_USBVM_REC_FILE_NODE(&recNode, sizeof(USBVM_REC_FILE_NODE), 1, g_usbvm_recordHandle[recordId].outputFp) != 1)
				{
					usbvm_closeRecFile(recordId);
					g_usbvm_recordHandle[recordId].fileTimName = 0;
					g_usbvm_recordHandle[recordId].fileSize = 0;
		...
		else
		{
			...
			sprintf(recFilePathName, "%s%d/%08x", 
				USB_REC_FILE_DIR, g_usbvm_recordHandle[recordId].endpt, g_usbvm_recordHandle[recordId].fileTimName);
