...
static VOICE_NOTIFY voiceNotifyTable[] =
{
	...
	{ 1, {40, 5,  8}, RTP_PAYLOAD_PCMU,  "recNotifyCustom_g711u", 0, 0},
	{ 1, {40, 5,  8}, RTP_PAYLOAD_PCMA,  "recNotifyCustom_g711a", 0, 0},   	
									...
									FILE **pfpVoiceNotify, unsigned int *pFileLen )
{
	...
	while (voiceNotifyTable[i].rtpPayLoadType != 0xff)
	{
		...
		{	
			...
			if (bUseCustom)
			{
				sprintf(ntyFilePathName, "%s%d/", USB_NOT_FILE_CUSTOM_DIR, endpt);
			}
			else
			{
						...
						case RTP_NTE_DTMF1:
							{
								...
								seq = 0;
								*/
								currentPos = sizeof(USBVM_REC_FILE_NODE);
								sleep(1);
								//ts += 1000 * 8;
								...
								*/
								...
								if(lockRet!=-1)
								{
									...
									else
									{
										...
										if (g_usbvm_recordHandle[recordId].payloadType == 0)
										{
											strcat(notifyFilePathName, "noRecordNotify_g711u");
										}
										else
										{
											strcat(notifyFilePathName, "noRecordNotify_g711a");
										}
