...
static int createWavHeader( FILE *fpWav, int formatTag, int channels, int samplerate, int resolution );
static int updateWavHeader( FILE *fpWav );
...
/* 
 * fn		static int createWavHeader( FILE *fpWav, int formatTag, int channels, int samplerate, int resolution )
...
/* 
 * fn		static int updateWavHeader( FILE *fpWav )
...
void usbvm_changeFormatToWav ( int endpt, int fileTimeName, BOOL bSleep)
{
	...
	sprintf(recFilePathName, "%s%d/%08x", USB_REC_FILE_DIR, endpt, fileTimeName);
...
/*20130618_update*/
	...
	if (formatMatch)
	{
		...
		if (access(wavFilePathName, F_OK) !=0)
		{
			...
			createWavHeader(fpWav, formatTag, PCM_CHANNEL_MONO, sampleRate, resolution);
...
/*20130618_update*/
	...
	if (rc == 0)
	{
		...
		sprintf(recFilePathName, "%s%d/%08x", USB_REC_FILE_DIR, endpt, node.fileTimName);
		if (access(recFilePathName, F_OK) == 0)
		{
			if (access(recFilePathName, R_OK | W_OK) != 0)
			{
