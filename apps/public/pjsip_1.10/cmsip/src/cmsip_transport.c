...
/* 
 * fn		extern int cmsip_send_systemlog(CMSIP_SYSTEMLOG_TYPE type, char* log)
 * brief	Send system log to CM.
 * details	
 *
 * param[in]	type	The type of system log
 * param[in]	log	The content of system log
 * param[out]	none
 *
 * return	int
 * retval	0	success
 *		-1	failure
 *
 * Note!!!: this function only can be called after g_cmsip_cliSockfd be created
 */
int cmsip_send_systemlog(CMSIP_SYSTEMLOG_TYPE type, const char* format, ...)
{
	va_list ap;
	char logBuf[LOG_CONTENT_LEN] = {0};
	...
	cmmlog(severity, LOG_VOIP, logBuf);
