			  ...
			  const pjsip_msg *msg)
{
	...
	len = pj_ansi_snprintf(info_buf, sizeof(info_buf), 
			       ...
			       pj_strnull(msg->line.req.method.name.ptr),
			       cseq->cseq, obj_name);
    } else {
	len = pj_ansi_snprintf(info_buf, sizeof(info_buf),
			       ...
			       pj_strnull(cseq->method.name.ptr),
			       cseq->cseq, obj_name);
    }
						 ...
						 int *port)
{
	...
	*port = 0;
	...
	else if (sel && sel->type == PJSIP_TPSELECTOR_LISTENER && sel->u.listener)
	{
	...
	{	
		...
		status = pjsip_tpmgr_acquire_transport(tpmgr, type, &remote, addr_len, NULL, &tp);
					       ...
					       pjsip_rx_data *rdata)
{
	...
	if (msg==NULL || !pj_list_empty(&rdata->msg_info.parse_err))
	{
		...
		len = pj_ansi_snprintf(tmp.ptr+tmp.slen, sizeof(buf)-tmp.slen,
				       ...
				       (int)err->hname.slen, pj_strnull(err->hname.ptr),
				       err->line, err->col);
	...
	/* For request: */
	/*ycw-pjsip:receive a request message*/
	...
	*/
	if (sel && sel->type == PJSIP_TPSELECTOR_TRANSPORT && sel->u.transport) 
	{
		...
		*tp = seltp;
		...
		TRACE_((THIS_FILE, "Transport %s acquired, use the specify sel", seltp->obj_name));
		return PJ_SUCCESS;
	...
	else
	{
		...
		CMSIP_PRINT("get transport(%p)", transport);

		if (transport == NULL)
		{
			unsigned flag = pjsip_transport_get_flag_from_type(type);
			const pj_sockaddr *remote_addr = (const pj_sockaddr*)remote;
			CMSIP_PRINT("flag(%d)", flag);
