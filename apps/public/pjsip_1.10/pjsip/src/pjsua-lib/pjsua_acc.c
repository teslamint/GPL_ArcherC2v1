				...
				for (i = 0; i < pjsua_var.acc_cnt; ++i)
	 			{
	 				tmp_acc_id = pjsua_var.acc_ids[i];
	 				acc = &pjsua_var.acc[tmp_acc_id];
					cmsip_send_systemlog(CMSIP_SYSTEMLOG_DEBUG, "match acc %d, valid %d", tmp_acc_id, acc->valid);
					if (acc->valid
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
						&& acc->regDuration>0 /*ycw-pjsip-regDuration*/
#	else
		...
		case PJSUA_STR_SIP_URI_NOMAL:
			{
				for (i = 0; i < pjsua_var.acc_cnt; ++i)
				{
					tmp_acc_id = pjsua_var.acc_ids[i];
					acc = &pjsua_var.acc[tmp_acc_id];
					cmsip_send_systemlog(CMSIP_SYSTEMLOG_DEBUG, "match2 acc %d valid %d", tmp_acc_id, acc->valid);
					if (acc->valid
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
						&& acc->regDuration>0
#	else
