 ...
 */
 /*ycw-pjsip:CMSIP_MSG*/
static int inv_send_callStatus(pjsip_dialog* dlg, pjsip_rx_data* rdata);
...
{	
	int statusCode;
	char contact[MAX_CONTACT_URI][MAX_URI_LEN] = {{0}};
	int cnt = 0;
	
	if (!dlg || !rdata)
	{
		PJ_LOG(1, (THIS_FILE, "dlg or rdata is NULL!!!"));
		return 0;
	}
	
	pjsip_msg *msg = rdata->msg_info.msg;	
	if (msg && msg->type == PJSIP_RESPONSE_MSG && rdata->msg_info.cseq->method.id == PJSIP_INVITE_METHOD)
	{		
		statusCode = msg->line.status.code;
		if (PJSIP_IS_STATUS_IN_CLASS(statusCode, 300))
		{
			int i = 0;

			const pjsip_hdr *hdr = msg->hdr.next;
			while (hdr != &msg->hdr && i < MAX_CONTACT_URI)
			{
				if (hdr->type == PJSIP_H_CONTACT)
				{
			   	const pjsip_contact_hdr *cn_hdr = (const pjsip_contact_hdr*)hdr;
					...
					{
						pj_status_t rc;
						rc = pjsip_uri_print(PJSIP_URI_IN_CONTACT_HDR, cn_hdr->uri, contact[i], MAX_URI_LEN);
		...
		return cmsip_send_callStatus(CMSIP_CALL_GENERIC, dlg->callIndex, dlg->seq, statusCode, contact, cnt);
	}
		   ...
		   pjsip_event *e)
{
	...
	CMSIP_PRINT("prev_state[%d] state[%d], event[%d]", prev_state, state, e? e->type:0);
    ...
    if (state == PJSIP_INV_STATE_CONFIRMED)
	 {
		if (pjmedia_sdp_neg_get_state(inv->neg)!=PJMEDIA_SDP_NEG_STATE_DONE)
		{
		    	...
		    	if (status == PJ_SUCCESS && bye)
			{
					...
					{
						(void)tsx;
						PJ_TODO(CANNOT_SEND_BYE_WHEN_SDP_NEG_ERROR);
						//TODO:do what? I need think........
					}
		    	}
...
static pj_bool_t mod_inv_on_rx_request(pjsip_rx_data *rdata)
{
	CMSIP_PRINT("---------mod inv on rx request------------------------");
    pjsip_method *method;
    pjsip_dialog *dlg;
    pjsip_inv_session *inv;
	pj_status_t status = PJ_SUCCESS; /*for t38--ycw*/
    ...
    if (method->id == PJSIP_ACK_METHOD && inv)
	 {
		    ...
		    inv->invite_tsx->state <= PJSIP_TSX_STATE_COMPLETED)
		{
		    ...
		    if (inv->state < PJSIP_INV_STATE_DISCONNECTED)
			 {
				status = inv_check_sdp_in_incoming_msg(inv, inv->invite_tsx, rdata);
					  ...
					  pjsip_inv_session **p_inv)
{
    ...
    inv->cause = (pjsip_status_code) 0;
	 ...
	 /*ycw-pjsip*/
	 inv->holdStat = PJSIP_HOLD_NONE;
	 inv->xferCallId = -1;
	 inv->origXferCallId = -1;

/*ycw-pjsip*/
#	if defined(SUPPORT_FAX_T38) && SUPPORT_FAX_T38!=0
	 inv->enableT38 = (options & PJSIP_INV_ENABLE_T38) ? PJ_TRUE : PJ_FALSE;
#	else
	 inv->enableT38 = PJ_FALSE;
#	endif
...
pjsip_rdata_sdp_info* pjsip_rdata_get_sdp_info(pjsip_rx_data *rdata)
{
    ...
    sdp_info = (pjsip_rdata_sdp_info*)rdata->endpt_info.mod_data[mod_inv.mod.id];
    if (sdp_info)
    {
		return sdp_info;
    }

    sdp_info = PJ_POOL_ZALLOC_T(rdata->tp_info.pool, pjsip_rdata_sdp_info);
...
#	if defined(SUPPORT_FAX_T38) && SUPPORT_FAX_T38!=0
	 if (*options & PJSIP_INV_ENABLE_T38)
	 {
	 	enableT38 = PJ_TRUE;
	 }
#	endif
    ...
    c_hdr = (pjsip_contact_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_CONTACT, NULL);
    if (!c_hdr || !c_hdr->uri)
	 {
		...
		if (l_sdp)
		{
	   	pjmedia_sdp_neg *neg;
	    	...
	    	status = pjmedia_sdp_neg_create_w_remote_offer(
			    rdata->tp_info.pool, l_sdp, r_sdp,
/*ycw-pjsip-enableT38*/
				enableT38,
			    &neg);
    ...
    sup_hdr = (pjsip_supported_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_SUPPORTED, NULL);
    if (sup_hdr)
	 {
		unsigned i;
    ...
    req_hdr = (pjsip_require_hdr*)pjsip_msg_find_hdr(msg, PJSIP_H_REQUIRE, NULL);
    if (req_hdr)
	 {
		unsigned i;
		const pj_str_t STR_100REL = { "100rel", 6};
	    ...
	    if (p_tdata)
		 {
			...
			for (i=0; i<unsupp_cnt; ++i)
		   {
			...
			if (h)
			{
		   	sup_hdr = (pjsip_supported_hdr*)pjsip_hdr_clone(rdata->tp_info.pool, h);
    ...
    if (rem_option & PJSIP_INV_REQUIRE_100REL)
	 #else
	 if ( rem_option & PJSIP_INV_SUPPORT_100REL || rem_option & PJSIP_INV_REQUIRE_100REL)
	 #endif
	 {
				      ...
				      pjsip_tx_data **p_tdata )
{
	...
	hdr = inv->dlg->inv_hdr.next;
		...
		{
	   	pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)pjsip_hdr_shallow_clone(tdata->pool, hdr));
	    hdr = hdr->next;
	}
    }
    ...
    if (status != PJ_SUCCESS)
    {
    	/*must release tdata mem. By YuChuwei*/
    	pjsip_tx_data_dec_ref(tdata);
	goto on_return;
    }
...
static pj_status_t inv_negotiate_sdp( pjsip_inv_session *inv )
{
    ...
    PJ_LOG(4, (inv->obj_name, "SDP negotiation done, status=%d", status));
						  ...
						  pjsip_rx_data *rdata)
{
    ...
    if (inv->neg == NULL || pjmedia_sdp_neg_get_state(inv->neg) == PJMEDIA_SDP_NEG_STATE_DONE) 
    {
		/* This is an offer. */
		PJ_LOG(4,(inv->obj_name, "Got SDP offer in %s", pjsip_rx_data_get_info(rdata)));
...
/*ycw-pjsip-enableT38*/
		...
		{
			status = (*mod_inv.cb.on_rx_offer)(inv, sdp_info->sdp);
			if (status != PJ_SUCCESS)
			{
				CMSIP_PRINT("-----on_rx_offer return %d-------", status);
				return status;
			}
		}
	 ...
	 else if (pjmedia_sdp_neg_get_state(inv->neg) == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER) 
    {
		 ...
		 */
		PJ_LOG(4,(inv->obj_name, "Got SDP answer in %s", pjsip_rx_data_get_info(rdata)));

		status = pjmedia_sdp_neg_set_remote_answer(inv->pool_prov, inv->neg, sdp_info->sdp);
		if (status != PJ_SUCCESS)
		{
...
/*ycw-pjsip-enableT38*/
		...
		else if (pjmedia_sdp_neg_get_state(inv->neg)== PJMEDIA_SDP_NEG_STATE_REMOTE_OFFER)
		{
	   	status = pjmedia_sdp_neg_set_local_answer(inv->pool_prov, inv->neg, local_sdp);
		}
		else
		{
    ...
    if (st_code/100==2 || (st_code/10==18 && st_code!=180))
	 {
		...
		/* Added by sirrain zhang */
#ifdef INCLUDE_USB_VOICEMAIL		
		/* Find the correct invite session and force negotiation */
		for (recId = 0; recId < PJSUA_USBVM_MAX_RECORDID; recId++)
		{
				 if ((g_usbvm_recordHandle[recId].cnxId != -1)
				 	&& (inv->dlg->callIndex == g_usbvm_recordHandle[recId].cnxId))
				 {
				 	break;
				 }
		}
		if (recId < PJSUA_USBVM_MAX_RECORDID)
		{
			usbvm_forceNegToG711(inv);
		}
#endif /* INCLUDE_USB_VOICEMAIL */
	   	status = inv_negotiate_sdp(inv);
	    	if (status != PJ_SUCCESS)
						...
						pjsip_tx_data **p_tdata)
{
    ...
    status = pjsip_timer_process_req(inv, rdata, &st_code2
    			...
    			, PJ_FALSE /*By YuChuwei, For Telefonica*/
#endif
    			);
    if (status != PJ_SUCCESS) {
	pj_status_t status2;
					...
					pjsip_tx_data **p_tdata )
{
    ...
    if (status != PJ_SUCCESS)
    {
    	CMSIP_PRINT("error. status[%d]", status);
		goto on_return;
    }
				 ...
				 const pj_str_t *reason, pjsip_event *e)
{
    ...
    pjsip_target_assign_status(inv->dlg->target_set.current, inv->dlg->pool, code, reason);
						...
						pjsip_event *e)
{
	 ...
	 */
	{
	    ...
	    status = pjsip_inv_send_msg(inv, tdata);
		 /*ycw-pjsip:CMSIP_REQUEST_SIP_AUTOCALLFORWARD*/
		 #if 1
	 	char reqUri[MAX_URI_LEN];
 		char toUri[MAX_URI_LEN];
 		pjsip_uri* uri = NULL;
 		pjsip_sip_uri* psipUri = NULL;
	 
 	 	/*ycw-pjsip:CMSIP_REQUEST_SIP_AUTOCALLFORWARD*/
 		uri = inv->dlg->target_set.current->uri;
 		psipUri = (pjsip_sip_uri*)pjsip_uri_get_uri(uri);
 		pjsip_uri_print(PJSIP_URI_IN_REQ_URI, psipUri, reqUri, MAX_URI_LEN);

 		pjsip_to_hdr* pto = (pjsip_to_hdr*)
		pjsip_msg_find_hdr(inv->invite_req->msg, PJSIP_H_TO, NULL);
 		psipUri = (pjsip_sip_uri*)pjsip_uri_get_uri(pto->uri);
 		pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, psipUri, toUri, MAX_URI_LEN);
		 
		 if (PJ_SUCCESS == status)
		 {
	 		cmsip_send_autocall_forward(inv->dlg->callIndex, inv->dlg->callIndex, reqUri, toUri);
		 }
		 else
		 {
		 	PJ_LOG(4, (THIS_FILE, "triggered INVITE(%s) FAIL,forward by %s.",
							reqUri, toUri));
		 }
		 #endif
	}
    ...
    if (new_contact)
	 {
		...
		contact_hdr = (pjsip_contact_hdr*)
		      pjsip_parse_hdr(inv->dlg->pool, &STR_CONTACT, tmp.ptr, tmp.slen, NULL);
		if (!contact_hdr)
		{
...
pj_status_t pjsip_inv_send_msg( pjsip_inv_session *inv, pjsip_tx_data *tdata)
{
	...
	if (tdata->msg->type == PJSIP_REQUEST_MSG)
	{
		...
		if (tdata->msg->line.req.method.id==PJSIP_INVITE_METHOD && inv->invite_tsx) 
		{
		    pjsip_tx_data_dec_ref(tdata);
		    pjsip_dlg_dec_lock(inv->dlg);
		    return PJ_EINVALIDOP;
		...
		status = pjsip_dlg_send_request(inv->dlg, tdata, mod_inv.mod.id, tsx_inv_data);
		if (status != PJ_SUCCESS)
		{
			return status;
		}
...
#ifdef INCLUDE_TFC_ES
    			,PJ_TRUE /*By YuChuwei, For Telefonica*/
#endif
    			);
    if (status != PJ_SUCCESS) {
...
static void handle_uac_call_rejection(pjsip_inv_session *inv, pjsip_event *e)
{
    ...
    pj_status_t status;
	 ...
	 if(inv->cancelling != PJ_TRUE)
	 {
		 inv_send_callStatus(dlg, rdata);
	 }
	 #endif
    ...
    if (PJSIP_IS_STATUS_IN_CLASS(tsx->status_code, 300))
	 ...
	 else if ((tsx->status_code==401 || tsx->status_code==407) &&	!inv->cancelling) 
    {
...
CMSIP_PRINT("==========process call========================");
    ...
    PJ_ASSERT_ON_FAIL(tsx && dlg, return);
	 ...
	 if (PJSIP_EVENT_TSX_STATE == e->type &&
	 	PJSIP_EVENT_RX_MSG == e->body.tsx_state.type &&
	 	inv->cancelling != PJ_TRUE &&
	 	(tsx!=inv->invite_tsx || tsx->state!=PJSIP_TSX_STATE_COMPLETED 
	 		|| tsx->status_code/100==2) && tsx->status_code != 100)
	 {
	 	 pjsip_rx_data* rdata = e->body.tsx_state.src.rdata;
		/*ycw-pjsip:CMSIP_SIP_CALLSTATUS */
		CMSIP_PRINT("---------recv status code (%d)", tsx->status_code);
		inv_send_callStatus(dlg, rdata);
	 }

	CMSIP_PRINT("---tsx(%p), invite_tsx(%p)-state(%d)---\n", tsx, inv->invite_tsx,tsx->state);
	   	 ...
	   	 if (inv->pending_cancel)
		 	{
				...
				/*ycw-pjsip-note*//*这儿应该发送release请求给cm?*//*不需要，因为这个操作本来
				就是cm发出的。*/
		    ...
		    if (dlg->remote.info->tag.slen)
			 {
				...
				{
			   	inv_handle_incoming_reliable_response(inv, e->body.tsx_state.src.rdata);
				}
		    }
			 else
			 {
...
# if 0
				if (tsx->send100ToCm)
				{
					char contact[1][MAX_URI_LEN];
					memset(contact, 0, 1 * MAX_URI_LEN);
					cmsip_send_callStatus(dlg->callIndex, dlg->channelId, 100, contact, 1);
					tsx->send100ToCm = PJ_FALSE;
				}		
#	endif
	    ...
	    if (tsx->status_code/100 == 2)
		 {
			...
			if (e->body.tsx_state.type == PJSIP_EVENT_RX_MSG)
			{
				/*ycw-pjsip:send CMSIP_SIP_CALLSTATUS*/
				if (inv->cancelling != PJ_TRUE)
				{
					inv_send_callStatus(dlg, rdata);
				}
		   	...
		   	/* Process session timer response. */
			    ...
			    inv_check_sdp_in_incoming_msg(inv, tsx, e->body.tsx_state.src.rdata);
			 }
...
CMSIP_PRINT("==========process call========================");
	...
	inv_handle_bye_response( inv, tsx, e->body.tsx_state.src.rdata, e);
	     ...
	     e->body.tsx_state.type == PJSIP_EVENT_RX_MSG) 
    {
    CMSIP_PRINT("handle stranded incoming CANCEL------------------------------");
	/*
	 * Handle stranded incoming CANCEL.
	 */
	pjsip_rx_data *rdata = e->body.tsx_state.src.rdata;
	pjsip_tx_data *tdata;
	pj_status_t status;
	...
	cmsip_send_callRelease(CMSIP_REQUEST_SIP_REQUIRECALLRELEASE, 
			dlg->callIndex);
...
CMSIP_PRINT("==========process call========================");
   ...
   PJ_ASSERT_ON_FAIL(tsx && dlg, return);
	 ...
	 */
	 CMSIP_PRINT("=======================Handle incoming re-INVITE=====================");
	if (tsx->state == PJSIP_TSX_STATE_TRYING) {
	    ...
	    if (inv->invite_tsx && inv->invite_tsx!=tsx)
		{
		 ...
		 /*ycw-pjsip.If this call has a related transfer call, it can not process
		  any re-INVITE request */
		 if (inv->xferCallId >= 0)
		 {
			int code;

			/*according to eyebeam's process*/
			code = 488;
	    ...
	    /* Create 2xx ANSWER */
		CMSIP_PRINT("==============create re-INVITE 200===========================");
	    status = pjsip_dlg_create_response(dlg, rdata, 200, NULL, &tdata);
	    if (status != PJ_SUCCESS)
		return;
	...
	{
		CMSIP_PRINT("=======transaction state is confirme===========");
	...
	CMSIP_PRINT("--------handle outgoing re-INVITE----------------");
	/*
	 * Handle outgoing re-INVITE
	 */
	 if (tsx->state == PJSIP_TSX_STATE_CALLING)
	 {
	 	CMSIP_PRINT("-----------state is calling---------------");
		   ...
		   tsx->status_code/100 == 2) 
	 {
	   ...
	   CMSIP_PRINT("-----------state is terminated and code=200---------------");
						...
						PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER)
	    {
		 ...
		 /*ycw-pjsip:send CMSIP_RESPONSE_SIP_HOLDSTATUS*/
		 switch (inv->holdStat)
		 {
		 case PJSIP_HOLD_SENT:
		 	cmsip_send_holdstatus(inv->dlg->callIndex, CMSIP_HOLD_SUCCESS);
		 break;
		 case PJSIP_HOLD_T38:
		 	cmsip_send_callStatus(CMSIP_CALL_T38, inv->dlg->callIndex, inv->dlg->seq,
					tsx->status_code, contactArray, 1);		
		 break;
		 case PJSIP_HOLD_PASSTHROUGH:
		 	cmsip_send_callStatus(CMSIP_CALL_PSTN, inv->dlg->callIndex, inv->dlg->seq,
					tsx->status_code, contactArray, 1);
		 break;
		 case PJSIP_HOLD_NONE: /*fall through*/
		 default:
		 	/*do nothing*/
		 break;
		 }
	...
	else if (tsx->status_code >= 300 && tsx->status_code < 700)
	{
	    ...
	    if (tsx == inv->invite_tsx)
	    {
		 ...
		 if (tsx->state < PJSIP_TSX_STATE_TERMINATED)
		 {
			 ...
			 {
			 case PJSIP_HOLD_SENT:
			 	cmsip_send_holdstatus(inv->dlg->callIndex, CMSIP_HOLD_FAIL);
			 break;
			 case PJSIP_HOLD_T38:
			 	cmsip_send_callStatus(CMSIP_CALL_T38, inv->dlg->callIndex, inv->dlg->seq,  
						tsx->status_code, contactArray, 1);		
