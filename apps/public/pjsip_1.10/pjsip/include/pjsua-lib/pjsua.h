...
/*ycw-pjsip*/
/*Include cmsip message define header*/
...
#   define PJSUA_ACC_MAX_PROXIES    1/*8*//*ycw-pjsip*/
#endif
#ifdef INCLUDE_USB_VOICEMAIL
	/* recordId should be the same as endpt number.If there has 2 endpts, we allow no more than 
		2 users to record at the same time.*/
	#ifdef NUM_FXS_CHANNELS
	#define PJSUA_USBVM_MAX_RECORDID NUM_FXS_CHANNELS
	#else
	#define PJSUA_USBVM_MAX_RECORDID 2
	#endif
	/* every recordId has at most 3 threads :
		[0] play thread, manage remote play and record;
		[1] receive rtp thread, get rtp pkts from sip;
		[2]	record thread, write data to file. */
	#define PJSUA_USBVM_MAX_THREADID 3		
#endif
...
#   define PJSUA_ACQUIRE_CALL_TIMEOUT 5000 /*2000*/
#endif

/**
* For incoming IP call, there is no account, so no ptime. Use this instead.
*/
#ifndef PJSUA_DEFAULT_LOCAL_PTIME
#	define PJSUA_DEFAULT_LOCAL_PTIME	20
#endif
...
#	endif
     ...
     */
    void (*on_call_transfer_request)(const pj_pool_t* pool, pjsua_call_id call_id,
				     const pjsip_uri *dst,
				     pjsip_status_code *code,
				     pj_bool_t* transferToSelf, /*by yuchuwei*/
...
/*ycw @ 2012-04-05: Because we don't use sip resolver*/
#ifdef INCLUDE_TFC_ES 
	/*Enable DNS Resolver, By yuchuwei, For Telefonica*/
    /**
...
/*yuchuwei@2012-4-5: we don't use global outbound proxy*/
#	if 0
    /**
...
#	endif
	...
	/*ycw-pjsip*/
	/*If ture, we add telephone-event pt to SDP, or need not add*/
	 pj_bool_t		has_telephone_event;

	/*ycw-pjsip*/
	unsigned reg_timeout;
	unsigned reg_retry_interval; /*reg retry intervals*/
...
typedef struct pjsua_transport_config
{
    ...
    pj_str_t		bound_addr;
	 ...
	 /*ycw-firewall*/
	 pj_str_t		bound_addr_netmask;

	/*ycw-pjsip: pjSIP Bound Network Interface name*/
	pj_str_t	bound_ifName;
...
#   define PJSUA_CALL_HOLD_TYPE_DEFAULT		PJSUA_CALL_HOLD_TYPE_RFC3264
 ...
 /*ycw-pjsip. type of this account is mapped to endpt*/ 
typedef enum
{
	MAPFXS_NONE = -1,
	MAPFXS_LINE1 = 0,
	...
	MAPFXS_IDLE = 2,
	MAPFXS_ALL = 3,
	MAPFXS_VOIP2PSTN = 4 /* VoIP to PSTN second stage dial */
}CMMAPFXS;


extern pj_bool_t app_exit_without_unregister;
...
typedef struct pjsua_acc_config
{
	/**
	*ycw-pjsip.
	*this index is the index in CM ACCOUNT.
	*/
	int cmAcctIndex;
...
#	endif
	...
	/* 
	 * brief	type of this account is mapped to endpt.
	 *			ycw-pjsip
	 */
	 CMMAPFXS mapEndpt;
	...
	/*implement RFC3325, For Telefonica, By YuChuwei*/
#ifdef INCLUDE_TFC_ES
	pj_bool_t isAuthed; /*This account has completed Authentication with proxy.
						Default:PJ_FALSE*/
	pj_bool_t supportNAI; /*support Network Asserted Identity.
							Default:PJ_FALSE*/
	pj_bool_t requestPriv; /*request privacy. Default:PJ_FALSE*/
	pj_str_t preferId;	/*value of P-Prefered-Identity*/
#endif
 ...
 * @param manual	To identify whether this register or unregister is trigger by user on the web UI.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
...
#   define PJSUA_MAX_CALLS	    5/*32*//*ycw-pjsip*/
#endif
...
/*ycw-pjsip*/
typedef enum PJSUA_STR_SIP_URI_TYPE
{
	PJSUA_STR_SIP_URI_UNKNOWN = 0,
	PJSUA_STR_SIP_URI_NOMAL = 1, /*normal format*/
	PJSUA_STR_SIP_URI_IP = 2,	/*ip*/
	PJSUA_STR_SIP_URI_TEL = 3	/*tel uri*/
}PJSUA_STR_SIP_URI_TYPE;
typedef struct PJSUA_STR_SIP_URI
{
	PJSUA_STR_SIP_URI_TYPE type;
 ...
 */
 /*ycw-pjsip. t38*/
 typedef unsigned pj_call_option;
#define pj_call_t38	0x0001
#define pj_call_anonymous      0x0002
/*ycw-pjsip-pstn*/
#define pj_call_pstn	0x0003
...
/*ycw-pjsip. t38*/
typedef unsigned pj_channel_mode;
#define pj_channel_idle         0x0000
#define pj_channel_voice        0x0001
#define pj_channel_passthrough  0x0002
#define pj_channel_t38          0x0004
...
#   define PJSUA_MAX_BUDDIES	    0/*256*//*ycw-pjsip*/
#endif
...
struct pjsua_media_config
{
	...
	/*ycw-pjsip.In 3631, each account has a ptime.So, delete this*/
	#if 0
    /**
...
/*ycw-pjsip*/
int find_audio_index(const pjmedia_sdp_session *sdp, pj_bool_t prefer_srtp);
...
/*ycw-pjsip. t38*/
typedef unsigned pjsua_media_fmt;
#define pjsua_faxr_passthrough 0x0001
#define pjsua_faxr_t38 0x0002
...
/*ycw-pjsip-usbvm*/
#ifndef PJSUA_MAX_CMENDPT
#ifdef NUM_FXS_CHANNELS
#define PJSUA_MAX_CMENDPT NUM_FXS_CHANNELS
#else
#define PJSUA_MAX_CMENDPT 2
#endif /* NUM_FXS_CHANNELS */
#endif /* PJSUA_MAX_CMENDPT */

typedef struct USBVMENDPTCONFIG
{
	pj_uint8_t customNotifyLocalRecordEnable; /* allow local endpt to record the cusomized notify */
	pj_uint8_t useCustomeNotifyForRemote; /* use the cusomized notify for remote recording */
	char pin[10]; /*the size is the same with CM's*/
} USBVMENDPTCONFIG;
