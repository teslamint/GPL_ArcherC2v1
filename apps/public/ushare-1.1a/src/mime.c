...
/*modify(HZG/HL): initial the string protocol[512],and call snprintf instead of sprintf for safety*/
char *mime_get_protocol (struct mime_type_t *mime)
{
  ...
  if (!mime)
    return NULL;
/*add by HZG/HL,2012-09-25
  even mime isn't a NULL pointer, mime->mime_protocol maybe NULL,when a mobile(such as SONY LT26I)
  try to play a video,this situation may happened and ushare will crash.add this check to avoid this bug*/
  if (mime->mime_protocol == NULL)
  {
	return NULL;
  }
/*end by HZG/HL */
  snprintf (protocol, 510, mime->mime_protocol);
  strcat (protocol, "*");
