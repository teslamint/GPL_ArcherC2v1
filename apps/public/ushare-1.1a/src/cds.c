                     ...
                     char *filter)
{
  ...
  char record[RECORD_BUFFER_SIZE] = {0};
  long long int size = 0;
  char * title = NULL;
  char * fullpath = NULL;
  dlna_profile_t * dlnap = NULL;
  struct mime_type_t *mime = NULL;
  
  if (entry < 0)
    return -1;

  child_count = getChildCount(entry);

	if (!getRecord(entry, record))
	{
		log_error("cds_browse_metadata : get record error.\n");
		return -1;
	}

	fullpath = record;
	title = record + OFFSET_TITLE;
	memcpy(&size, record + OFFSET_SIZE, sizeof(long long int));
	dir = *(record + OFFSET_DIR);
	memcpy(&parentid, record + OFFSET_PARENT_ID, sizeof(int));
	mime = getRecordMimeType(dir, fullpath);
	dlnap = getDlna_profile(fullpath, dir);
	if (!mime && !dlnap)
	{
		USHARE_ERROR("not media file.\n");
		return -1;
	}
	getURL(entry, fullpath, url, dir);
	
  if (child_count == -1) /* item : file */
  {
#ifdef HAVE_DLNA
    extern struct ushare_t *ut;
#endif /* HAVE_DLNA */
    ...
    char *protocol =
      ...
      dlna_write_protocol_info (DLNA_PROTOCOL_INFO_TYPE_HTTP,
                                ...
                                ut->dlna_flags, dlnap) :
#endif /* HAVE_DLNA */
      mime_get_protocol (mime);
    
    didl_add_header (out);
#ifdef HAVE_DLNA
    dlnap ?
      didl_add_item (out, entry, parentid, "false",
                     dlna_profile_upnp_object_item (dlnap),
                     title, protocol, size, url, filter) :
#endif /* HAVE_DLNA */
      didl_add_item (out, entry, parentid, "false",
                     mime->mime_class, title,
                     protocol, size, url, filter);
    
    didl_add_footer (out);
    free (protocol);
    
    for (c = index; c < MIN (index + count, child_count); c++)
      result_count++;
  }
  else  /* container : directory */
  {
    didl_add_header (out);
    didl_add_container (out, entry, parentid, child_count,
                        "true", "true", title, mime->mime_class);
    didl_add_footer (out);
...
cds_browse_directchildren (struct action_event_t *event,
                           ...
                           int count, int entry, char *filter)
{
  int childs = 0;
  int i = 0;
  int result_count = 0;
  ...
  char * fullpath = NULL;
  int child_count = 0;
  int subChild_count = 0;
  struct mime_type_t *mime = NULL;
  dlna_profile_t * dlnap = NULL;
  
  child_count = getChildCount(entry);
  //USHARE_DEBUG("id: %d  childCount: %d \n", entry, child_count);
  if (child_count == -1) /* item : file */
    return -1;
  ...
  for (i = 0; i <= childs; i++)
  {
    ...
    {
    	if((id = getChild(entry, id, index + 1)) < 0)
    	{
    		log_error("cds_browse_directchildren : get child error.\n");
    		return -1;
    	}
		index = 0;
		//USHARE_DEBUG("child id: %d\n", id);
		if (!getRecord(id, record))
		{
			log_error("cds_browse_directchildren : get record error.\n");
			return -1;
		}
		fullpath = record;
		title = record + OFFSET_TITLE;
		...
		getURL(id, fullpath, url, dir);
		//USHARE_DEBUG("fullpath:%s\ntitle:%s\nurl:%s\nsize:%lld  dir:%d\n",
		//				fullpath, title, url, size, dir);
      if ((subChild_count = getChildCount(id)) >= 0) /* container */
        didl_add_container (out, id, entry,
                            subChild_count, "true", NULL,
                            title, mime->mime_class);
      else /* item */
      {
#ifdef HAVE_DLNA
        extern struct ushare_t *ut;
...
#ifdef HAVE_DLNA
          dlnap ?
          didl_add_item (out, id, entry,
                         "true", dlna_profile_upnp_object_item (dlnap),
                          title, protocol, size, url, filter) : 
#endif /* HAVE_DLNA */
          didl_add_item (out, id,
                         entry, "true", mime->mime_class,
                         title, protocol, size, url, filter);
        free (protocol);
      }
      result_count++;
...
#if 1
  USHARE_DEBUG("catch request:\n index: %d\n count: %d\n"
  			   "id: %d\nflag: %s\nfilter: %s\n sort: %d\n",
  			   index, count, id, flag, filter, sort_criteria);
#endif

if (id > 0 && id < ut->starting_id)
{
	id = ut->starting_id;
}
/*
...
*/
  ...
  if (metadata)
    result_count =
      cds_browse_metadata (event, out, index, count, id, filter);
  else
    result_count =
      cds_browse_directchildren (event, out, index, count, id, filter);
  free (filter);
...
static bool
matches_search(char *search_criteria, dlna_profile_t * dlnap, struct mime_type_t *mime)
{
  ...
  if (derived_from && mime
      && !strncmp (mime->mime_class, keyword, strlen (keyword)))
    result = true;
  else if (protocol_contains && strstr (protocol, keyword))
    result = true;
  else if (mime &&
           !strcmp (mime->mime_class, keyword))
    result = true;
  free (protocol);
  ...
  if (and_clause)
    return (result &&
            matches_search (and_clause + strlen (SEARCH_AND) -1, dlnap, mime));
...
cds_search_directchildren_recursive(struct buffer_t *out, int count,
                                     int entry, char *filter, int *index,
                                     char *search_criteria)
{
  ...
  for (i = 0; i <= childs; i++)
  {
      ...
      /* only fetch the requested count number or all entries if count = 0 */
    {
		...
		if (!getRecord(id, record))
		{
			log_error("cds_search_directchildren_recursive : get record error.\n");
			return -1;
		}
       ...
       if ((subChild_count = getChildCount(id)) >= 0)  /* container */
      {
        ...
        new_count = cds_search_directchildren_recursive
          (out, (count == 0) ? 0 : (count - result_count),
           id, filter, index, search_criteria);
        result_count += new_count;
      }
      else /* item */
      {
        if (matches_search (search_criteria, dlnap, mime))
        {
			if (*index)
			{
				(*index)--;
                           ...
                           char *filter, char *search_criteria)
{
  ...
  for (i = 0; i <= childs; i++)
  {
      ...
      /* only fetch the requested count number or all entries if count = 0 */
    {
		...
		if (!getRecord(id, record))
		{
			log_error("cds_search_directchildren : get record error.\n");
			return -1;
		}
  ...
  {
  	sprintf (tmp, "%d", result_count + base_count);
  }
  else
  {
  	sprintf (tmp, "%d", result_count + base_count + 1);
  }
  upnp_add_response (event, SERVICE_CDS_DIDL_TOTAL_MATCH, tmp);
...
*/
  ...
  result_count =
    cds_search_directchildren (event, out, index, count, id,
...
/* Added by LI CHENGLONG , 2011-Nov-18.*/
