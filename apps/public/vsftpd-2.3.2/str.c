...
/* for fopen etc. by HouXB, 20Apr11 */
#include <stdio.h>
...
{	
	...
	int i = 0;	
	int flag = 0;
	char ftpPermission='0';  //initialize by chz
	
	/* modify by chz */
	//fp = fopen("/var/vsftp/etc/passwd", "r");
	...
	else if (ftpPermission == '0')
	{
		die("you have no permission to access");
	}
...
//add by chz to get our global passwd entry 
char *ftp_getpwnam(char *userName)
{
	FILE *fp=NULL;
	int i=0;
	int len=0;
	...
	for (i=0; i<5; i++)
	{
		if (!strncmp(passwdBuf[i], userName, len) && (passwdBuf[i][len] == ':'))
		{
			return passwdBuf[i];
		}
	}
