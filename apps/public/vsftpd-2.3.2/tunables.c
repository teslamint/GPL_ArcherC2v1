...
tunables_load_defaults()
{
  ...
  tunable_hide_ids = 0;
  //modify by chz for standalone mode
  //tunable_listen = 0;
  tunable_listen = 1;
  //end modify
  ...
  /* modify by chz for 8968 */
  install_str_setting(VSFTP_DIR, &tunable_secure_chroot_dir);
  install_str_setting("ftp", &tunable_ftp_username);
  install_str_setting("root", &tunable_chown_username);
  install_str_setting("/var/vsftp/log/xferlog", &tunable_xferlog_file);
  install_str_setting("/var/vsftp/log/vsftpd.log", &tunable_vsftpd_log_file);
  install_str_setting(".message", &tunable_message_file);
  install_str_setting("nobody", &tunable_nopriv_user);
  install_str_setting(0, &tunable_ftpd_banner);
  install_str_setting(VSFTP_ETC_DIR"/vsftpd.banned_emails", &tunable_banned_email_file);
  install_str_setting(VSFTP_ETC_DIR"/vsftpd.chroot_list", &tunable_chroot_list_file);
  install_str_setting("ftp", &tunable_pam_service_name);
  install_str_setting("ftp", &tunable_guest_username);
  install_str_setting(VSFTP_ETC_DIR"/vsftpd.user_list", &tunable_userlist_file);
  ...
  install_str_setting(VSFTP_ETC_DIR"/vsftpd.email_passwords",
                      &tunable_email_password_file);
  install_str_setting(VSFTP_CERTS_DIR"/vsftpd.pem",
