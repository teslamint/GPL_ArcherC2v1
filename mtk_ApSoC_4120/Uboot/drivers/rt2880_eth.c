...
void enable_auto_negotiate(void)
{
	...
	*(volatile u_long *)(RALINK_ETH_SW_BASE+0x3400) |= (0x1 << 17);

#endif
#if defined (P5_MAC_TO_PHY_MODE) 
	*(volatile u_long*)(RALINK_ETH_SW_BASE+0x3500) &= ~(0x1 << 15);
	*(volatile u_long *)(RALINK_ETH_SW_BASE+0x3500) |= (0x1 << 17);
...
void rt_gsw_init(void)
{
...
#ifdef TP_MODEL_C2V1
	RALINK_REG(RALINK_ETH_SW_BASE+0x7014) = 0x1fd0000c; 
	//RALINK_REG(RALINK_ETH_SW_BASE+0x7014) |= (1 << 20);
	udelay (10000 * 50 * 1);/* 0.5s */
	RALINK_REG(0xb0000034) = (0x1<<24); //reset embeded switch 
	udelay (10000 * 50 * 1);/* 0.5s */
	RALINK_REG(0xb0000034) = (0x0<<24); 
	...
	u8 phyBase = 16;
	mii_mgr_write(phyBase + 1, 31, 0x4000); //global, page 4
	mii_mgr_write(phyBase + 1, 17, 0x7444);
	if(is_BGA){
		mii_mgr_write(phyBase + 1, 19, 0x0114);
	}else{
		mii_mgr_write(phyBase + 1, 19, 0x0117);
	}	
	mii_mgr_write(phyBase + 1, 22, 0x10cf);
	mii_mgr_write(phyBase + 1, 25, 0x6212);
	mii_mgr_write(phyBase + 1, 26, 0x0777);
	mii_mgr_write(phyBase + 1, 29, 0x4000);
	mii_mgr_write(phyBase + 1, 28, 0xc077);
	mii_mgr_write(phyBase + 1, 24, 0x0000);

	mii_mgr_write(phyBase + 1, 31, 0x3000); //global, page 3  
	mii_mgr_write(phyBase + 1, 17, 0x4838);

	mii_mgr_write(phyBase + 1, 31, 0x2000); //global, page 2

	if(is_BGA){
	    mii_mgr_write(phyBase + 1, 21, 0x0515);
	    mii_mgr_write(phyBase + 1, 22, 0x0053);
	    mii_mgr_write(phyBase + 1, 23, 0x00bf);
	    mii_mgr_write(phyBase + 1, 24, 0x0aaf);
	    mii_mgr_write(phyBase + 1, 25, 0x0fad);
	    mii_mgr_write(phyBase + 1, 26, 0x0fc1);
	}else{
	    mii_mgr_write(phyBase + 1, 21, 0x0517);
	    mii_mgr_write(phyBase + 1, 22, 0x0fd2);
	    mii_mgr_write(phyBase + 1, 23, 0x00bf);
	    mii_mgr_write(phyBase + 1, 24, 0x0aab);
	    mii_mgr_write(phyBase + 1, 25, 0x00ae);
	    mii_mgr_write(phyBase + 1, 26, 0x0fff);
	}
	mii_mgr_write(phyBase + 1, 31, 0x1000); //global, page 1  
	mii_mgr_write(phyBase + 1, 17, 0xe7f8);

	mii_mgr_write(phyBase + 1, 31, 0x8000); //local, page 0
	mii_mgr_write(phyBase + 0, 30, 0xa000);
	mii_mgr_write(phyBase + 1, 30, 0xa000);
	mii_mgr_write(phyBase + 2, 30, 0xa000);
	mii_mgr_write(phyBase + 3, 30, 0xa000);
#if defined(P4_MAC_TO_NONE_MODE)	
	mii_mgr_write(phyBase + 4, 30, 0xa000);
#endif

	mii_mgr_write(phyBase + 0, 4, 0x05e1);
        mii_mgr_write(phyBase + 1, 4, 0x05e1);
	mii_mgr_write(phyBase + 2, 4, 0x05e1);
	mii_mgr_write(phyBase + 3, 4, 0x05e1);
#if defined(P4_MAC_TO_NONE_MODE)	
	mii_mgr_write(phyBase + 4, 4, 0x05e1);
#endif

	mii_mgr_write(phyBase + 1, 31, 0xa000); //local, page 2
	mii_mgr_write(phyBase + 0, 16, 0x1111);
	mii_mgr_write(phyBase + 1, 16, 0x1010);
	mii_mgr_write(phyBase + 2, 16, 0x1515);
	mii_mgr_write(phyBase + 3, 16, 0x0f0f);
#if defined(P4_MAC_TO_NONE_MODE)	
	mii_mgr_write(phyBase + 4, 16, 0x1313);
#endif
	...
	for (i=0; i<4; i++) {
#endif
	    mii_mgr_read(phyBase + i, 0, &phy_val);
	    phy_val |= 1<<9; //restart AN
	    mii_mgr_write(phyBase + i, 0, phy_val);
	}
#endif 
		
	/* add end  */
#endif
	RALINK_REG(RALINK_ETH_SW_BASE+0x3600) = 0x5f33b;//CPU Port6 Force Link 1G, FC ON
	RALINK_REG(RALINK_ETH_SW_BASE+0x0010) = 0x7f7f7fe0;//CPU exist in port 6
#if defined (P5_RGMII_TO_MAC_MODE)
	//RALINK_REG(RALINK_ETH_SW_BASE+0x3500) = 0x5e33b; ////(P5, Force mode, Link Up, 1000Mbps, Full-Duplex, FC ON)
	/* add by wanghao  */
	RALINK_REG(RALINK_ETH_SW_BASE+0x3500) = 0x5f33b; ////(P5, Force mode, Link Up, 1000Mbps, Full-Duplex, FC ON)
	/* add end  */
	RALINK_REG(0xb0000060) &= ~(1 << 9); //set RGMII to Normal mode
	/* add by wanghao  */
	//RALINK_REG(RALINK_ETH_SW_BASE+0x7014) = 0x1fc0000c; 
       RALINK_REG(0xb0000060) &= ~(3<<7);
	/* add end  */
	...
	RALINK_REG(RALINK_ETH_SW_BASE+0x3400) = 0x5f33b; ////(P4, Force mode, Link Up, 1000Mbps, Full-Duplex, FC ON)
