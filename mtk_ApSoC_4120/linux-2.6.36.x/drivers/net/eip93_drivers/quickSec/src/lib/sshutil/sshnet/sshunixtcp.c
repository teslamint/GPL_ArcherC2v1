...
SshOperationHandle ssh_tcp_connect_ip(SshIpAddr remote_address,
  ...
  if (local_address || local_port)
    {
      ...
      if (SSH_IP_IS4(&ipaddr))
	    ...
	    (unsigned long)(inet_network(ssh_csstr(local_address)));
          ...
          sinaddr.sin_addr.s_addr =
	    (unsigned long)(inet_addr(ssh_csstr(local_address)));
                          ...
                          void *context)
{
  ...
  return ssh_tcp_make_listener_one_ip(ipaddr, port, params, callback, context);
}
...
{
  SshIpAddrStruct ip[1];
  if (!ssh_tcp_get_ip_addresses(stream, NULL, NULL, ip, NULL))
    return FALSE;
...
{
  SshUInt16 port;
  if (!ssh_tcp_get_ip_addresses(stream, NULL, NULL, NULL, &port))
    return FALSE;
...
{
  SshIpAddrStruct ip[1];
  if (!ssh_tcp_get_ip_addresses(stream, ip, NULL, NULL, NULL))
    return FALSE;
  ssh_ipaddr_print(ip, buf, buflen);
  return TRUE;
}
...
{
  SshUInt16 port;
  if (!ssh_tcp_get_ip_addresses(stream, NULL, &port, NULL, NULL))
    return FALSE;
  ssh_snprintf(buf, buflen, "%u", port);
  return TRUE;
}
