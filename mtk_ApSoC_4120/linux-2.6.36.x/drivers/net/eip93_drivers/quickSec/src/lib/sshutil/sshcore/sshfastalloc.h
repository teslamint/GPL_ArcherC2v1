...
/**  Reserve room for allocating at least `objects' new objects.  If this call
     returns TRUE, then the allocator has reserved room for `objects' new 
     objects and it is guaranteed that they can be allocated without failures.
     If the call returns FALSE then there isn't currently enough memory
     available. */
