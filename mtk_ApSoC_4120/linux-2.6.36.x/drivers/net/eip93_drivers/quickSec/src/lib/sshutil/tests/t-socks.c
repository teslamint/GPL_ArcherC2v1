...
#if 0
  /* Change success condition if these are to be returned. */
  (const unsigned char *) "2.1",  /* 2.0.0.1 */
  (const unsigned char *) "1.2.3" /* 1.2.0.3 */
#endif
...
#if 0
  /* Depends on the test environment having ssh at services table */
...
{
  unsigned int pass, i;
  SshBufferStruct buffer, buffer2;
          ...
          for (i = 0; i + 1 < len; i++)
            {
              ...
              if (ssh_socks_server_parse_open(&buffer2, &socksreturn) !=
                  SSH_SOCKS_TRY_AGAIN)
                ssh_fatal("ssh_socks_server_parse_open fails in partial data "
                          "(should return try_again)");
            }
          ssh_buffer_append(&buffer2, p + i, 1);
          if (ssh_socks_server_parse_open(&buffer2, &socksreturn) !=
              SSH_SOCKS_SUCCESS)
            ssh_fatal("ssh_socks_server_parse_open fails for partial data "
       ...
       pass++)
    {
      ...
      socksinfo->ip =
        ssh_xstrdup(ip_numbers[pass % (sizeof(ip_numbers) /
                                   sizeof(ip_numbers[0]))]);
      socksinfo->port =
        ssh_xstrdup(port_invalid_numbers[pass
                                         % (sizeof(port_invalid_numbers) /
      ...
      if (ssh_socks_client_generate_open(&buffer, socksinfo) ==
          SSH_SOCKS_SUCCESS)
        ssh_fatal("ssh_socks_client_generate_open success "
                  "(should fail, port)");
      socksinfo->command_code = (pass % 4) + 90;
      socksinfo->socks_version_number = 4;
      if (ssh_socks_server_generate_reply(&buffer, socksinfo) ==
          SSH_SOCKS_SUCCESS)
        ssh_fatal("ssh_socks_server_generate_reply success "
                  "(should fail, port)");
      ssh_socks_free(&socksinfo);
    }
  if (ssh_buffer_len(&buffer) != 0)
    ssh_fatal("some of the failed ssh_socks_*_generate_* function wrote "
              "something to buffer, size != 0");
