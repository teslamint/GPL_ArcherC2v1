...
#define SSH_DEBUG_MODULE "ModeTest"
...
Boolean ecb_alloc(char *name)
{
  ...
  if (!ssh_cipher_supported(name) || !ssh_cipher_supported(ecb_name))
    return FALSE; 
  ...
  blocklen = ssh_cipher_get_block_length(ecb_name);
