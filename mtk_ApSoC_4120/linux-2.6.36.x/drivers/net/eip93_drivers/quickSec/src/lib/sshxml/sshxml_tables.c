...
#define SSH_XML_NUM_ITEMS(array)        \
const size_t array ## _num_items = (sizeof(array) / sizeof(array[0]))

const SshXmlCharRangeStruct ssh_xml_base_char[] =
{
...
SSH_XML_NUM_ITEMS(ssh_xml_base_char);

const SshXmlCharRangeStruct ssh_xml_ideographic[] =
{
  {0x4E00, 0x9FA5}, {0x3007, 0x3007}, {0x3021, 0x3029},
};

SSH_XML_NUM_ITEMS(ssh_xml_ideographic);

const SshXmlCharRangeStruct ssh_xml_combining_char[] =
{
...
SSH_XML_NUM_ITEMS(ssh_xml_combining_char);

const SshXmlCharRangeStruct ssh_xml_digit[] =
{
...
SSH_XML_NUM_ITEMS(ssh_xml_digit);

const SshXmlCharRangeStruct ssh_xml_extender[] =
{
...
SSH_XML_NUM_ITEMS(ssh_xml_extender);

Boolean
ssh_xml_char_in_array(SshXmlChar ch, const SshXmlCharRangeStruct array[],
                      size_t num_items)
{
  size_t i;
  ...
  for (i = 0; i < num_items; i++)
    if (ch < array[i].min)
      return FALSE;
    else if (array[i].min <= ch && ch <= array[i].max)
      return TRUE;
...
/* Predefined entities. */
const SshXmlPreDefinedEntityStruct ssh_xml_pre_defined_entities[] =
{
  {"lt",        "&#60;"},
  {"gt",        "&#62;"},
  {"amp",       "&#38;"},
...
SSH_XML_NUM_ITEMS(ssh_xml_pre_defined_entities);
...
} ssh_xml_input_encodings[] =
{
  {"UTF-8",             SSH_XML_INPUT_ENC_UTF_8},
  {"UTF-16",            SSH_XML_INPUT_ENC_UNKNOWN},
  {"UTF-16BE",          SSH_XML_INPUT_ENC_UTF_16_BE},
  {"UTF-16LE",          SSH_XML_INPUT_ENC_UTF_16_LE},

  {"ISO-10646-UCS-2",   SSH_XML_INPUT_ENC_UNKNOWN},
  {"csUnicode",         SSH_XML_INPUT_ENC_UNKNOWN},

  {"ISO-10646-UCS-4",   SSH_XML_INPUT_ENC_UNKNOWN},
  {"csUCS4",            SSH_XML_INPUT_ENC_UNKNOWN},

  {"ISO-8859-1:1987",   SSH_XML_INPUT_ENC_ISO_8859_1},
  {"iso-ir-100",        SSH_XML_INPUT_ENC_ISO_8859_1},
  {"ISO_8859-1",        SSH_XML_INPUT_ENC_ISO_8859_1},
  {"ISO-8859-1",        SSH_XML_INPUT_ENC_ISO_8859_1},
  {"latin1",            SSH_XML_INPUT_ENC_ISO_8859_1},
  {"l1",                SSH_XML_INPUT_ENC_ISO_8859_1},
  {"IBM819",            SSH_XML_INPUT_ENC_ISO_8859_1},
  {"CP819",             SSH_XML_INPUT_ENC_ISO_8859_1},
  {"csISOLatin1",       SSH_XML_INPUT_ENC_ISO_8859_1},

  {"ANSI_X3.4-1968",    SSH_XML_INPUT_ENC_US_ASCII},
  {"iso-ir-6",          SSH_XML_INPUT_ENC_US_ASCII},
  {"ANSI_X3.4-1986",    SSH_XML_INPUT_ENC_US_ASCII},
  {"ISO_646.irv:1991",  SSH_XML_INPUT_ENC_US_ASCII},
  {"ASCII",             SSH_XML_INPUT_ENC_US_ASCII},
  {"ISO646-US",         SSH_XML_INPUT_ENC_US_ASCII},
  {"US-ASCII",          SSH_XML_INPUT_ENC_US_ASCII},
  {"us",                SSH_XML_INPUT_ENC_US_ASCII},
  {"IBM367",            SSH_XML_INPUT_ENC_US_ASCII},
  {"cp367",             SSH_XML_INPUT_ENC_US_ASCII},
  {"csASCII",           SSH_XML_INPUT_ENC_US_ASCII},
};

SSH_XML_NUM_ITEMS(ssh_xml_input_encodings);


Boolean
ssh_xml_input_encoding(const char *name, SshXmlInputEncoding *encoding_return)
{
  size_t i;

  for (i = 0; i < ssh_xml_input_encodings_num_items; i++)
    if (strcasecmp(ssh_xml_input_encodings[i].name, name) == 0)
      {
        if (ssh_xml_input_encodings[i].encoding)
          *encoding_return = ssh_xml_input_encodings[i].encoding;
        return TRUE;
      }
