...
typedef struct
{
    ...
    Byte buffered_data[Z_BUFSIZE];/* buffer contain compressed data to
                                     be writ*/
    uLong dosDate;
    uLong crc32;
} curfile_info;
     ...
     int level;
{
  ...
  ziplocal_putValue_inmemory(zi->ci.central_header+34,
                             (uLong)0,2); /*disk nm start*/
  ...
  if (err==ZIP_OK)
    err = ziplocal_putValue(zi->filezip,(uLong)20,2);/* version
                                                        needed to
                                                        extract */
  if (err==ZIP_OK)
  ...
  if (err==ZIP_OK)
    err = ziplocal_putValue(zi->filezip,(uLong)0,4); /* compressed size,
                                                        unknown */
  if (err==ZIP_OK)
    err = ziplocal_putValue(zi->filezip,(uLong)0,4); /* uncompressed
                                                        size, unknown */
    ...
    while ((err==ZIP_OK) && (zi->ci.stream.avail_in>0))
    {
        if (zi->ci.stream.avail_out == 0)
        {
            if (fwrite(zi->ci.buffered_data,
                       (uInt)zi->ci.pos_in_buffered_data,1,zi->filezip)
                !=1)
              err = ZIP_ERRNO;
            zi->ci.pos_in_buffered_data = 0;
            zi->ci.stream.avail_out = (uInt)Z_BUFSIZE;
        ...
        if (zi->ci.method == Z_DEFLATED)
        {
            ...
            zi->ci.pos_in_buffered_data += (uInt)(zi->ci.stream.total_out -
                                                  uTotalOutBefore) ;
     ...
     zipFile file;
{
  ...
  ziplocal_putValue_inmemory(zi->ci.central_header+16,
                             (uLong)zi->ci.crc32,4); /*crc*/
  ziplocal_putValue_inmemory(zi->ci.central_header+20,
                             (uLong)zi->ci.stream.total_out,4); /*compr
                                                                  size*/
  ziplocal_putValue_inmemory(zi->ci.central_header+24,
                             (uLong)zi->ci.stream.total_in,4); /*uncompr
                                                                 size*/
  ...
  if (err==ZIP_OK)
    {
      ...
      if (err==ZIP_OK)
        err = ziplocal_putValue(zi->filezip,
                                (uLong)zi->ci.crc32,4); /* crc 32, unknown */
    ...
    const char* global_comment;
{
    ...
    if (err==ZIP_OK) /* total number of entries in the central dir on
                        this disk */
    ...
    if (err==ZIP_OK) /* offset of start of central directory with
    ...
    if ((err==ZIP_OK) && (size_global_comment>0))
        if (fwrite(global_comment,(uInt)size_global_comment,1,zi->filezip)
            !=1)
                err = ZIP_ERRNO;
    fclose(zi->filezip);
    TRYFREE(zi);
