      ...
      if (ret == 0)
        {
          if (read_count != send_count)
            ssh_fatal("server1_read eof received, "
                      "read_count %ld send_count %ld",
...
int main(int ac, char **av)
{
  ...
  ssh_snprintf(lpath1, sizeof (lpath1), "/tmp/lstr1.%x", (unsigned)ssh_rand());
  ssh_snprintf(lpath2, sizeof (lpath2), "/tmp/lstr2.%x", (unsigned)ssh_rand());
