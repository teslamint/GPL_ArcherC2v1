...
#ifdef LINUX_MODULE_DEPENDS_HAS_USED_ATTRIBUTE
__used
#else /* LINUX_MODULE_DEPENDS_HAS_USED_ATTRIBUTE */
__attribute_used__ 
#endif /* LINUX_MODULE_DEPENDS_HAS_USED_ATTRIBUTE */
...
ssh_interceptor_stop(SshInterceptor interceptor)
{
  ...
  /* Wait for all timeouts to shutdown */
  SSH_TIMEOUT_QUEUE_LOCK(interceptor);

  if (interceptor->num_timeout_callbacks)
    {
      SSH_TIMEOUT_QUEUE_UNLOCK(interceptor);
      SSH_DEBUG(SSH_D_ERROR, ("%d timeout callbacks pending, can't stop",
                              interceptor->num_timeout_callbacks));
      return FALSE;
    }
...
ssh_interceptor_uninit_kernel_services(void)
{
  ...
  ssh_interceptor_context->is_timeout_shutdown = TRUE;

  /* Empty timeout freelist */
  ssh_kernel_timeout_freelist_uninit();

  /* Free locks */
  ssh_kernel_mutex_free(ssh_interceptor_context->interceptor_lock);
  ssh_interceptor_context->interceptor_lock = NULL;
  ssh_kernel_mutex_free(ssh_interceptor_context->timeout_lock);
  ssh_interceptor_context->timeout_lock = NULL;
...
ssh_interceptor_init_kernel_services(void)
{
  ...
  ssh_interceptor_context->timeout_lock = ssh_kernel_mutex_alloc();
  ...
  if (!ssh_interceptor_packet_freelist_init(ssh_interceptor_context))
    {
      printk(KERN_ERR
             "SafeNet Quicksec packet processing engine failed to start "
             "(out of memory).\n");
      goto error;
    }
