/**
   ...
   Routines to get and set routing information. Used by the engine.
...
#define SSH_DEBUG_MODULE "SshInterceptorRoute"
				  ...
				  SshInterceptorRouteResult result)
{
  ...
  result->mtu = SSH_LINUX_DST_MTU(&SSH_RT_DST(rt));
				  ...
				  SshInterceptorRouteResult result)
{
  ...
  if (selector & SSH_INTERCEPTOR_ROUTE_KEY_FLAG_TRANSFORM_APPLIED)
    {
      if (dst->child == NULL)
	if (interceptor_route_create_child_dst(dst) == NULL)
	  SSH_DEBUG(SSH_D_FAIL, ("Could not create child dst_entry for dst %p",
				 dst));
    }
