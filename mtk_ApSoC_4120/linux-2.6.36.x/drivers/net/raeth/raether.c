...
int __init rather_probe(struct net_device *dev)
{
	...
	spiflash_ioctl_read(addr.sa_data, 0x7DF100, 6);
	printk("Read MAC from flash(0x7DF100) %02x-%02x-%02x-%02x-%02x-%02x\n", 
...
void rt_gsw_init(void)
{
	...
	*(unsigned long *)(0xb0000060) &= ~(1 << 9); //set RGMII to Normal mode
	...
	//*(unsigned long *)(0xb0000060) &= ~(3<<7);/* delete now  */
...
extern int isEthForwardEnable();
extern int enableEthForward();
extern int disableEthForward();
...
int rt_mt7620_chkLan(void)
{
	...
	{
		regData = *(unsigned long *)(RALINK_ETH_SW_BASE + 0x3008 + (count * 0x100));
		if (regData & 0x1)
		{
			return 0;
		}
...
int rt_mt7620_chkWan(void)
{
	u32 regData = 0;

	regData = *(unsigned long *)(RALINK_ETH_SW_BASE + 0x3008);
	if (regData & 0x1)
	{
		return 0;
	}
...
/* add by wanghao */
	...
	fp_gpioTestLan = rt_rt8367_chkLan;
    fp_ethTestWan = rt_rt8367_chkWan;
    
#elif CONFIG_TP_MODEL_C20iV1

	fp_gpioTestLan = rt_mt7620_chkLan;
    fp_ethTestWan = rt_mt7620_chkWan;
