...
extern int br_igmp_mc_forward(struct net_bridge *br, struct sk_buff *skb,const  unsigned char *dest,int forward, int clone);
...
extern int br_igmp_mc_fdb_add(struct net_bridge *br, struct net_bridge_port *prt, const unsigned char *dest, unsigned char *host, int mode, struct in_addr *src);
