...
int32_t PpeParseLayerInfo(struct sk_buff * skb)
{
	...
	if(is_multicast_ether_addr(&eth->h_dest[0])) {
		...
		/* C20i 去掉MCAST走HNAT, 避免Igmp Snooping功能失效, yuanshang, 2014-2-20 */
		return 1;	
#else
...
static void getACLTable(int index)
{
	unsigned int i, value;
	value = (0x80004000 + index);  //w_acl entry 0
	RegWrite(RALINK_ETH_SW_BASE  + REG_ESW_VLAN_VTCR, value);
	...
	value = RegRead(RALINK_ETH_SW_BASE + REG_ESW_VLAN_VAWD1);
	printk("REG_ESW_VLAN_VAWD1 value is 0x%x\n\r", value);
	value = RegRead(RALINK_ETH_SW_BASE + REG_ESW_VLAN_VAWD2);
	printk("REG_ESW_VLAN_VAWD2 value is 0x%x\n\r", value);
	
}

static void getAclControl(int index)
{
	unsigned int i, value;
	value = (0x80008000 + index);  //w_acl control rule  RULE_NUM
	RegWrite(RALINK_ETH_SW_BASE + REG_ESW_VLAN_VTCR, value);
	...
	value = (0x8000a000 + index);  //w_acl rule action control RULE_NUM
	RegWrite(RALINK_ETH_SW_BASE + REG_ESW_VLAN_VTCR, value);
...
static void SetAclFwd(uint32_t Ebl)
{
	...
	SetAclControl(0x208, 5, 0xAF80);//Multicast inc. PPE ??????
	...
	SetAclControl(0x808, 0xb, 0xAF80);//Multicast inc. PPE ??????
