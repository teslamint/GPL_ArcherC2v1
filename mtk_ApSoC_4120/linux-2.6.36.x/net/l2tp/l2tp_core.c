...
/*support hw_nat, added by Zhu Haiming, 30Dec13.*/
#if  defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE)
...
/*support hw_nat, added by Zhu Haiming, 30Dec13.*/
	...
	{
		//printk("[%s]%d: l2tp xmit.\n", __FUNCTION__, __LINE__);
		printk("[%s]%d: magic: %x, ai: %x, alg: %x sp: %x\n", __FUNCTION__, __LINE__,
				FOE_MAGIC_TAG(skb), FOE_AI(skb), FOE_ALG(skb), FOE_SP(skb));
	}
#endif
	if (IS_MAGIC_TAG_VALID(skb))
    {
